...
> **版本主题**: v1.5.4 是 "Python 安全热修复版"

---

> 每次修改代码后，必须验证所有相关项目。标记 `[R]` 表示必须验证，`[O]` 表示可选验证。
> **重要**：打包后回归测试前，必须确认安装包 API 地址已取证验证为 `http://121.41.179.197:8000` (v1.5.1+)。
> 
> **历史版本 API 基线**:
> - v1.5.4: `http://121.41.179.197:8000` (当前正式版本，Python 安全热修复版)
> - v1.5.3: `http://121.41.179.197:8000` (已发布但 CI Python 安全门禁未全绿，保留为历史记录)
> - v1.5.2: `http://121.41.179.197:8000` (已发布但 CI npm audit 门禁未全绿，保留为历史记录)
> - v1.5.1: `http://121.41.179.197:8000` (上一稳定版本)
> - v1.5.0: `https://auth.xiuer.work` (旧基线，历史记录)
>
