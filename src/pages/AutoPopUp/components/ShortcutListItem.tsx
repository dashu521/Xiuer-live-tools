...
const ShortcutListItem: React.FC<ShortcutListItemProps> = ({
  ...
  // 当 shortcut prop 变化时（如账号切换），同步更新本地状态
  useEffect(() => {
    setNewKey(shortcut.key)
    setNewGoodsIds(shortcut.goodsIds)
    setCtrlKey(shortcut.ctrl ?? false)
    setAltKey(shortcut.alt ?? false)
    setShiftKey(shortcut.shift ?? false)
    setNewGoodsIdInput('')
    if (!isNew) {
      setIsEditing(false)
    }
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="flex flex-wrap gap-1.5 max-w-xs">
              {shortcut.goodsIds.map((id, index) => (
                <span key={`display-${index}`} className="bg-muted px-2 py-0.5 rounded text-sm">
                  {id}
                </span>
              ))}
